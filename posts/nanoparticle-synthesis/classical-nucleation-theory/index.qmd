...
- Surface energy (interfacial energy): $\gamma$ in J/m$^2$
- Volumetric driving force: $\Delta g_v$ in J/m$^3$
- Free energy of formation of a nucleus: $\Delta G_T(r)$ in J/nucleus ($T$ denotes 'Total')
- Critical free energy of formation of a nucleus: $\Delta G_c$ in J/nucleus ($c$ denotes 'critical')

Sign convention:

- $\Delta g_v = g_{v,s} - g_{v,l}$ is the solid–liquid Gibbs free energy difference per unit volume.
...
- Supersaturation form (solution nucleation, per particle convention):
  ...
  \Delta \mu = k_B T \ln S
  $$
where $\Delta \mu$ is the chemical potential difference (J/particle) and $S$ is supersaturation ($\frac{c_0}{c_\text{eq}}$).
...
Let $g_{v,s}$ and $g_{v,l}$ denote the Gibbs free energy per unit volume of solid and liquid, respectively (J/m$^3$). The *bulk (volume)* contribution is

$$
\Delta G_V(r) = \frac{4}{3}\pi r^3 (g_{v,s} - g_{v,l})
...
> —Wolfgang Pauli[@Brantley1999]

If $\gamma$ is the solid–liquid interfacial energy (J/m$^2$), (also referred to as interfacial energy) then the *surface* contribution is
...
For small $r$, the surface term dominates and $\Delta G_T(r)$ increases with size. For sufficiently large $r$, the bulk term dominates and $\Delta G_T(r)$ decreases with size. This competition produces a maximum in $\Delta G_T(r)$, which is the *nucleation barrier*, shown in @fig-bulk-surface.
...
where $\Delta h_f$ is the enthalpy of fusion per unit volume (J/m$^3$) and $\Delta T = T_f - T$ (positive below $T_f$). This approximation is valid for $\lvert\Delta T\rvert \ll T_f$.
...
$$
v_m = \frac{V_M}{N_A}, \qquad k_B = \frac{R}{N_A}
$$
...
where $k_B$ is Boltzmann’s constant and $J_0$ is a kinetic prefactor[^j_0]. Using the CNT barrier height,
...
[^j_0]: $J_0$ is the product of the number of nucleation sites, $N_S$, the rate at which molecules attach to the nucleus $j$, and the Zeldovich factor $Z$ which gives the probability that a nucleus at the top of the energy barrier to form a new phase rather than dissolve.
...
In many real syntheses, nucleation occurs on container walls, impurities, seeds, or other surfaces (i.e., heterogeneously).

::: {#fig-contact-angle fig-cap="Schematic of a liquid drop showing the quantities in the Young equation. [TikZ source](https://github.com/faithefairy/fai-notes/blob/main/posts/nanoparticle-synthesis/classical-nucleation-theory/assets/spherical_cap.tex)."}
![](assets/spherical_cap.svg){width=80%}
:::

The contact angle, $\theta$, is the angle formed where a liquid meets a solid surface. More precisely, it’s measured between the tangent to the liquid–vapor interface and the tangent to the solid–liquid interface at the point where they intersect. This angle is used to describe how well a liquid wets a solid surface and is related to the interfacial tensions through the [Young equation](https://en.wikipedia.org/wiki/Wetting#Ideal_solid_surfaces), which describes mechanical equilibrium at the three-phase contact line.

$$
\gamma^{\alpha \delta} = \gamma^{\beta \delta} + \gamma^{\alpha \beta} \cos(\theta)
$$

Doing some geometry reveals that for a spherical-cap nucleus (phase $\beta$) the volume is

$$
V_\beta = \frac{\pi r^3}{3} (2 + \cos \theta)(1-\cos\theta)^2
$$

and the surface area of the nucleus in contact with the solvent

$$
A_{\alpha\beta} = 2 \pi r^2 (1 - \cos \theta)
$$

and the cross-sectional area shared by the spherical cap and the substrate is

$$
A_{\alpha\delta} = A_{\beta\delta} = \pi \sin^2\theta r^2
$$

Similarly to homogeneous nucleation, both a bulk and surface term contribute the free energy. However, we also have a new third component $\Delta G_{\gamma^{\beta \delta}}$ that accounts for the interfacial energy associated with replacing part of the original solid-liquid interface between the substrate (container wall) and the solvent to a solid-solid interface between the substrate and the nucleus. So, we have

$$
\Delta G_V^\text{hetero}(r) = \Delta g_v V_\beta = \Delta g_v \frac{\pi r^3}{3}(2 + \cos \theta)(1 - \cos \theta)^2
$$

and the surface term is

$$
\Delta G_S^\text{hetero} = \gamma^{\alpha\beta} A_{\alpha\beta} = 2 \pi r^2 \gamma^{\alpha\beta}(1-\cos\theta)
$$

:::{.callout-note}
The energy penalty due to surface between the nucleus and the solvent is lower since the interface created has a smaller area.
:::

And finally, the third component describing the energy of replacing an interface is

$$
\Delta G_{\text{replace}} = (\gamma^{\beta\delta} - \gamma^{\alpha\delta}) A_{\beta\delta} = (\gamma^{\beta\delta} - \gamma^{\alpha \delta}) \pi r^2 \sin^2\theta
$$

Combining all of these together we get,

$$
\Delta G_T^\text{hetero} = \Delta g_v V_\beta + \gamma^{\alpha\beta}A_{\alpha\beta} + (\gamma^{\beta\delta} - \gamma^{\alpha\delta}) A_{\beta\delta}
$$

We can simplify this by assuming that we are at Young equilibrium,

$$
\Delta G_T^\text{hetero} = \Delta g_v V_\beta + \gamma^{\alpha\beta} A_{\alpha\beta} - \gamma^{\alpha\beta} \cos(\theta) A_{\beta\delta} 
$$

and upon expanding we get that

$$
\Delta G_T^\text{hetero} = \Delta G_T^\text{homo} f(\theta)
$$
...
r = np.linspace(0.0, 20e-9, 1000)
r_nm = r * 1e9

delta_G_bulk = (4.0/3.0) * np.pi * r**3 * delta_Gv
delta_G_surface = 4.0 * np.pi * r**2 * gamma
delta_G_homo = delta_G_bulk + delta_G_surface

r_c = -2.0 * gamma / delta_Gv
delta_G_homo_c = (16.0 * np.pi * gamma**3) / (3.0 * delta_Gv**2)

fig, ax = plt.subplots(figsize=(7, 5))
(line_homo,) = ax.plot(r_nm, delta_G_homo, lw=2, label="Homo")
(line_hetero,) = ax.plot(r_nm, delta_G_homo, lw=2, label="Hetero")

pt_homo = ax.scatter([r_c * 1e9], [delta_G_homo_c], zorder=3, s=50)
pt_hetero = ax.scatter([r_c * 1e9], [delta_G_homo_c], zorder=3, s=50)
...
ax.set_xlim(0.0, 1.8 * r_c * 1e9)
ax.set_ylim(0.015 * delta_G_bulk.min(), 0.08 * delta_G_surface.max())
ax.legend(frameon=False)
fig.tight_layout()

thetas = np.r_[np.linspace(0.0, 180.0, 181), np.linspace(180.0, 0.0, 181)[1:]]
th = np.deg2rad(thetas)
f_vals = 0.25 * (2.0 + np.cos(th)) * (1.0 - np.cos(th))**2
...
def update(i):
    ...
    pt_hetero.set_offsets([[r_c * 1e9, f * delta_G_homo_c]])

    txt.set_text(fr"$\theta$ = {thetas[i]:.0f}°   $f(\theta)$ = {f:.2f}")
...
::: {#fig-theta-effect}
<video src="assets/homo_hetero.mp4"
       autoplay
       loop
       muted
       playsinline
...
Effect of contact angle on free energy of heterogeneous nucleation under Young elimination.
:::

We notice in @fig-theta-effect that heterogeneous nucleation has an energy barrier lower than that of an equivalent homogeneous nucleation. Under the Young-equilibrium assumption, the heterogenous free energy reduces to the homogeneous form scaled by a geometric factor $f(\theta)$ and the critical radius remains the same as the homogeneous case. This occurs because the contact angle modifies the geometry of the nucleus, reducing the interfacial area and thus the barrier height, but does not change the volumetric driving force ($\Delta g_v$) that determines $r_c$.
...
| Quantity\\Formulation | Volumetric $\Delta g_v$ | Supercooling $\Delta T$ | Supersaturation $S$ |
|---|:---:|:---:|:---:|
| Free energy $\Delta G_T(r)$ | $\tfrac{4}{3} \pi r^3 \Delta g_v + 4 \pi r^2 \gamma$ | $-\dfrac{4 \pi \rho \hat{q}_f}{3 T_s} \Delta T r^3 + 4 \pi r^2 \gamma$ | $-\dfrac{k_B T}{v_m}\ln S + 4 \pi r^2 \gamma$ |
| Critical radius $r_c$ | $\dfrac{2\gamma}{|\Delta g_v|}$ | $\dfrac{2\gamma T_f}{\Delta h_{f,v}\,\Delta T}$ | $\dfrac{2\gamma v_m}{k_B T \ln S}$ |
| Barrier height $\Delta G_c$ | $\dfrac{16\pi\gamma^3}{3(\Delta g_v)^2}$ | $\dfrac{16\pi\gamma^3 T_f^2}{3(\Delta h_{f,v})^2(\Delta T)^2}$ | $\dfrac{16\pi\gamma^3 v_m^2}{3 k_B^3 T^3 (\ln S)^2}$ |
| Nucleation rate $J$ | $J_0 \exp\!\left(-\dfrac{\Delta G_c}{k_B T}\right)$ | same | same |
...
| Result | Expression |
|---|---|
| Total free energy $G_T$ | $\Delta g_v V_\beta + \gamma^{\alpha\beta}A_{\alpha\beta} + (\gamma^{\beta\delta} - \gamma^{\alpha\delta}) A_{\beta\delta}$
| Barrier scaling | $\Delta G_c^{\text{hetero}} = f(\theta)\,\Delta G_c^{\text{homo}}$ |
| Critical radius | $r_c^{\text{hetero}} = r_c^{\text{homo}}$ |
| Geometric factor | $f(\theta) = \dfrac{(2+\cos\theta)(1-\cos\theta)^2}{4}$ |
| Bounds | $0 \le f(\theta) \le 1,\quad 0^\circ \le \theta \le 180^\circ$ |
